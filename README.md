...
- 计划模组名：`Create Crystal Clear`。
- 计划包名：`io.liujiewentt.*`（后续待定）。
- 计划`modId`：使用原始仓库的（见下文）。
...
- 文档存储在`docs`中。如果是自己主机上遇到的不具有普适性的问题、不会所有人共同存在的问题，应写入到对应的加了`-personal`文件名后缀的文档。
- 参考代码库存储在`referring_repos`中，AI可以自行将想要参考的仓库克隆到此目录下，可以自行进行任何非破坏性Git操作。
- 重建项目独立一个仓库，存储在`repo`中，并作为一个完整的独立项目。
- 临时文件、个人专用文件存储在会被Git忽略的`keep_local`目录下。
  - 环境初始化：`keep_local\init\`，存放JAVA等环境变量覆盖脚本。
...
```
- 读取`文档编写指导.md`和`工作计划.md`，读取必要的相关文档，然后向用户确认计划，然后开始工作。
- 注意许可问题，避免直接使用不兼容MIT协议的代码。
- 验证阶段不要擅自修改东西，防止验证失败不好找原因。
- 编写文档的时候注意隐私问题。
- 编写文档时若有AIGC水印，尝试调用去除的技能，可以形成一个临时脚本，存放在`keep_local/temp/aigc_watermark_remove/`下，并在最后提示用户手动执行。
```
